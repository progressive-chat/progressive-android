...
class UnifiedPushHelper @Inject constructor(
    ...
    suspend fun storeCustomOrDefaultGateway(
        ...
        if (UnifiedPush.getDistributor(context) == context.packageName) {
            unifiedPushStore.storePushGateway(
                    gateway = mdmService.getData(
                            ...
                            defaultValue = stringProvider.getString(chat.progressive.app.config.R.string.pusher_http_url),
                    )
            )
            onDoneRunnable?.run()
            return
        ...
        val gateway = stringProvider.getString(chat.progressive.app.config.R.string.default_push_gateway_http_url)
