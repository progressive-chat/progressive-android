...
class PushersManager @Inject constructor(
    ...
    suspend fun testPush() {
        ...
        currentSession.pushersService().testPush(
                ...
                stringProvider.getString(chat.progressive.app.config.R.string.pusher_app_id),
    ...
    suspend fun enqueueRegisterPusherWithFcmKey(pushKey: String): UUID {
        return enqueueRegisterPusher(
                pushKey = pushKey,
                gateway = mdmService.getData(MdmData.DefaultPushGatewayUrl, stringProvider.getString(chat.progressive.app.config.R.string.pusher_http_url))
        )
    }
    ...
    private suspend fun createHttpPusher(
            ...
            pushkey = pushKey,
            appId = stringProvider.getString(chat.progressive.app.config.R.string.pusher_app_id),
    ...
    suspend fun unregisterPusher(pushKey: String) {
        val currentSession = activeSessionHolder.getSafeActiveSession() ?: return
        currentSession.pushersService().removeHttpPusher(pushKey, stringProvider.getString(chat.progressive.app.config.R.string.pusher_app_id))
    }
