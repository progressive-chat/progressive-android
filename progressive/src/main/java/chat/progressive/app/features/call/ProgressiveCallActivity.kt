...
import chat.progressive.app.databinding.ActivityCallBinding
import chat.progressive.app.features.call.audio.MicrophoneAccessService
import chat.progressive.app.features.call.dialpad.CallDialPadBottomSheet
...
import chat.progressive.app.features.call.utils.EglUtils
import chat.progressive.app.features.call.webrtc.ScreenCaptureAndroidService
import chat.progressive.app.features.call.webrtc.ScreenCaptureServiceConnection
...
class ProgressiveCallActivity :
    ...
    private fun enterPictureInPictureIfRequired(): Boolean = withState(callViewModel) {
        if (!it.isVideoCall) {
            ...
            val aspectRatio = Rational(
                    resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.call_pip_width),
                    resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.call_pip_height)
            )
    ...
    private fun configureCallInfo(state: ProgressiveCallViewState, blurAvatar: Boolean = false) {
        state.callInfo?.opponentUserItem?.let {
            val colorFilter = ContextCompat.getColor(this, chat.progressive.lib.ui.styles.R.color.bg_call_screen_blur)
