...
class DisplayableEventFormatter @Inject constructor(
    ...
    private fun simpleFormat(senderName: String, body: CharSequence, appendAuthor: Boolean): CharSequence {
        return if (appendAuthor) {
            span {
                text = senderName
                textColor = colorProvider.getColorFromAttribute(chat.progressive.lib.ui.styles.R.attr.vctr_content_primary)
            }
                    .append(": ")
                    .append(body)
        } else {
    ...
    private fun formatVoiceBroadcastEvent(event: Event, isDm: Boolean, senderName: String): CharSequence {
        return if (event.asVoiceBroadcastEvent()?.isLive == true) {
            span {
                drawableProvider.getDrawable(R.drawable.ic_voice_broadcast, colorProvider.getColor(chat.progressive.lib.ui.styles.R.color.palette_vermilion))?.let {
                    image(it)
                    +" "
                }
                span(stringProvider.getString(CommonStrings.voice_broadcast_live_broadcast)) {
                    textColor = colorProvider.getColor(chat.progressive.lib.ui.styles.R.color.palette_vermilion)
                }
            }
        } else {
