...
import chat.progressive.app.databinding.FragmentLockScreenBinding
...
import chat.progressive.app.features.pin.lockscreen.views.LockScreenCodeView
...
class LockScreenFragment :
    ...
    private fun setupTitleView(titleView: TextView, isConfirmation: Boolean, configuration: LockScreenConfiguration) = with(titleView) {
        text = if (isConfirmation) {
            configuration.newCodeConfirmationTitle ?: getString(chat.progressive.lib.ui.styles.R.string.lockscreen_confirm_pin)
        } else {
            configuration.title ?: getString(chat.progressive.lib.ui.styles.R.string.lockscreen_title)
        }
    }
