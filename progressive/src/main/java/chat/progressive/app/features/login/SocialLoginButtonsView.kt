...
class SocialLoginButtonsView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
            ...
            val button: MaterialButton = cachedViews[identityProvider.id]
                    ?: when (identityProvider.brand) {
                        SsoIdentityProvider.BRAND_GOOGLE -> {
                            MaterialButton(context, null, chat.progressive.lib.ui.styles.R.attr.vctr_social_login_button_google_style)
                        }
                        SsoIdentityProvider.BRAND_GITHUB -> {
                            MaterialButton(context, null, chat.progressive.lib.ui.styles.R.attr.vctr_social_login_button_github_style)
                        }
                        SsoIdentityProvider.BRAND_APPLE -> {
                            MaterialButton(context, null, chat.progressive.lib.ui.styles.R.attr.vctr_social_login_button_apple_style)
                        }
                        SsoIdentityProvider.BRAND_FACEBOOK -> {
                            MaterialButton(context, null, chat.progressive.lib.ui.styles.R.attr.vctr_social_login_button_facebook_style)
                        }
                        SsoIdentityProvider.BRAND_TWITTER -> {
                            MaterialButton(context, null, chat.progressive.lib.ui.styles.R.attr.vctr_social_login_button_twitter_style)
                        }
                        SsoIdentityProvider.BRAND_GITLAB -> {
                            MaterialButton(context, null, chat.progressive.lib.ui.styles.R.attr.vctr_social_login_button_gitlab_style)
                        }
                        else -> {
                            // TODO Use iconUrl
                            MaterialButton(context, null, com.google.android.material.R.attr.materialButtonOutlinedStyle).apply {
    ...
    init {
        ...
        val typedArray = context.theme.obtainStyledAttributes(attrs, chat.progressive.lib.ui.styles.R.styleable.SocialLoginButtonsView, 0, 0)
        val modeAttr = typedArray.getInt(chat.progressive.lib.ui.styles.R.styleable.SocialLoginButtonsView_signMode, 2)
        mode = when (modeAttr) {
            0 -> Mode.MODE_SIGN_IN
            1 -> Mode.MODE_SIGN_UP
