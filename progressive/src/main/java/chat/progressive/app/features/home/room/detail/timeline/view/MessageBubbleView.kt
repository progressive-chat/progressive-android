...
import chat.progressive.app.core.resources.getLayoutDirectionFromCurrentLocale
import chat.progressive.app.core.utils.DimensionConverter
import chat.progressive.app.databinding.ViewMessageBubbleBinding
import chat.progressive.app.features.home.room.detail.timeline.style.TimelineMessageLayout
import chat.progressive.app.features.home.room.detail.timeline.style.shapeAppearanceModel
...
class MessageBubbleView @JvmOverloads constructor(
    ...
    init {
        ...
        context.withStyledAttributes(attrs, chat.progressive.lib.ui.styles.R.styleable.MessageBubble) {
            isIncoming = getBoolean(chat.progressive.lib.ui.styles.R.styleable.MessageBubble_incoming_style, false)
        }
    }
        ...
        bubbleDrawable.apply {
            ...
            this.fillColor = if (isPseudoBubble) {
                ...
                val backgroundColorAttr =
                        if (isIncoming) {
                            chat.progressive.lib.ui.styles.R.attr.vctr_message_bubble_inbound
                        } else {
                            chat.progressive.lib.ui.styles.R.attr.vctr_message_bubble_outbound
                        }
    ...
    private fun TimelineMessageLayout.Bubble.toggleMessageOverlay() = apply {
        if (addMessageOverlay) {
            val timeColor = ContextCompat.getColor(context, chat.progressive.lib.ui.styles.R.color.palette_white)
            ...
            val timeColor = ThemeUtils.getColor(context, chat.progressive.lib.ui.styles.R.attr.vctr_content_tertiary)
            views.messageTimeView.setTextColor(timeColor)
    ...
    private fun TimelineMessageLayout.Bubble.setMargins() = apply {
        if (isIncoming) {
            views.messageEndGuideline.updateLayoutParams<LayoutParams> {
                marginEnd = resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.chat_bubble_margin_end)
            }
            views.messageStartGuideline.updateLayoutParams<LayoutParams> {
                marginStart = resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.chat_bubble_margin_start)
            }
        } else {
            views.messageEndGuideline.updateLayoutParams<LayoutParams> {
                marginEnd = resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.chat_bubble_margin_start)
            }
            views.messageStartGuideline.updateLayoutParams<LayoutParams> {
                marginStart = resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.chat_bubble_margin_end)
            }
        }
    }
