...
import chat.progressive.app.databinding.ViewSecurityRecommendationBinding
...
class SecurityRecommendationView @JvmOverloads constructor(
    ...
    init {
        ...
        context.obtainStyledAttributes(
                attrs,
                chat.progressive.lib.ui.styles.R.styleable.SecurityRecommendationView,
                0,
                0
        ).use {
            setTitle(it)
    ...
    private fun setTitle(typedArray: TypedArray) {
        val title = typedArray.getString(chat.progressive.lib.ui.styles.R.styleable.SecurityRecommendationView_recommendationTitle)
        views.recommendationTitleTextView.text = title
    }

    private fun setDescription(typedArray: TypedArray) {
        val description = typedArray.getString(chat.progressive.lib.ui.styles.R.styleable.SecurityRecommendationView_recommendationDescription)
        setDescription(description)
    }

    private fun setImage(typedArray: TypedArray) {
        val imageResource = typedArray.getResourceId(chat.progressive.lib.ui.styles.R.styleable.SecurityRecommendationView_recommendationImageResource, 0)
        val backgroundTint = typedArray.getColor(chat.progressive.lib.ui.styles.R.styleable.SecurityRecommendationView_recommendationImageBackgroundTint, 0)
