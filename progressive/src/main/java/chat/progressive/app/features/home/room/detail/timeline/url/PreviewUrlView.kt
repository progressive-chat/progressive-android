...
package chat.progressive.app.features.home.room.detail.timeline.url
...
import chat.progressive.app.databinding.ViewUrlPreviewBinding
...
class PreviewUrlView @JvmOverloads constructor(
    ...
    init {
        setupView()
        radius = resources.getDimensionPixelSize(chat.progressive.lib.ui.styles.R.dimen.preview_url_view_corner_radius).toFloat()
        cardElevation = 0f
    }
    ...
    override fun renderMessageLayout(messageLayout: TimelineMessageLayout) {
        when (messageLayout) {
            is TimelineMessageLayout.Default -> {
                val backgroundColor = ThemeUtils.getColor(context, chat.progressive.lib.ui.styles.R.attr.vctr_system)
