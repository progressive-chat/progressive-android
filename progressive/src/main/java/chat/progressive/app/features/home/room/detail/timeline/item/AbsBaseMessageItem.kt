...
import chat.progressive.app.core.extensions.getDrawableAsSpannable
...
import chat.progressive.app.features.home.room.detail.timeline.view.TimelineMessageLayoutRenderer
import chat.progressive.app.features.reactions.widget.ReactionButton
...
abstract class AbsBaseMessageItem<H : AbsBaseMessageItem.Holder>(@LayoutRes layoutId: Int) : BaseEventItem<H>(layoutId) {
    ...
    private fun createReactionTextView(holder: H): TextView {
        return TextView(ContextThemeWrapper(holder.view.context, chat.progressive.lib.ui.styles.R.style.TimelineReactionView)).apply {
            background = getDrawable(context, R.drawable.reaction_rounded_rect_shape_off)
            TextViewCompat.setTextAppearance(this, chat.progressive.lib.ui.styles.R.style.TextAppearance_Vector_Micro)
            setTypeface(typeface, Typeface.BOLD)
            setTextColor(ThemeUtils.getColor(context, chat.progressive.lib.ui.styles.R.attr.vctr_content_secondary))
        }
    }
